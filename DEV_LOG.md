...
### 2026-04-03 — 天气市场页前端自动刷新增强

**完成内容**

- `WeatherMarketPage`：
  - 保留快照拉取结构，但改成更完整的自动刷新机制
  - 前台标签页每 `30s` 自动重拉一次快照
  - 后台标签页降频到每 `2min` 重拉一次，减少无效请求
  - 切回标签页、窗口重新聚焦、网络恢复时立即重拉
  - 页面头部新增“自动刷新 / 当前状态 / 上次同步”可视提示
- `WeatherMarketDetailPage`：
  - 同步接入与列表页一致的自动刷新机制
  - 详情页停留时也会跟随后端快照变化自动更新
  - 头部新增“自动刷新 / 当前状态 / 上次同步”提示
- 部署：
  - `npm run build` 验证
  - 部署到 Vercel 生产环境
...
- 这次只完成了前端自动重拉；页面是否真正“实时”，仍取决于服务端快照有没有更新
- 目前前端拉取的仍是 `/data/weather-market-snapshot.json` 静态快照，不是运行时 API

**下一步**

- 把天气市场数据源从静态 JSON 迁到运行时 API
- 让前端轮询 `/api/weather-market-snapshot`，而不是依赖重新构建部署
- 后端增加缓存层，避免每次请求都现场抓取所有外部数据

---

### 2026-04-03 — 天气市场手续费口径接入前端

**完成内容**

- `build-weather-market-snapshot.mjs`：
  - 快照新增透传 `entryFeeUsdc`
  - 持仓时间线的 `ENTRY / REBAL_IN` 事件同步透传 `entry_fee_usdc`
- `WeatherMarketPage`：
  - 当前浮动盈亏改为净口径：`当前市值 - 本金 - 买入手续费`
  - 当前余额改为净口径：`当前市值 - 买入手续费`
  - 有手续费时显示“已计入买入手续费 X USDC”
- `WeatherMarketDetailPage`：
  - 当前余额 / 结束余额改为净口径
  - 详情页增加“买入手续费”字段
  - 详细动作中的进场事件显示买入手续费
...
- 若后端快照尚未提供 `entry_fee_usdc`，前端会自动按 `0` 回退
- 后端一旦把手续费写入 `positions.json` 并推送快照，前端会自动切换到净口径

---

### 2026-04-03 — 天气市场详情页新增最新净利润

**完成内容**

- `WeatherMarketDetailPage` 新增“最新净利润”指标
- 当前持仓口径：
  - 历史换仓已实现净盈亏
  - 加上当前这腿的未实现净盈亏
- 历史持仓口径：
